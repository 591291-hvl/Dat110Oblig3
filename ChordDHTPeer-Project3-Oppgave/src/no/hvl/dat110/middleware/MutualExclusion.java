...
public class MutualExclusion {
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for(Message peer : activenodes) {
			...
			NodeInterface stub = Util.getProcessStub(peer.getNodeIP(), peer.getPort());
	...
	public void onMutexRequestReceived(Message message) throws RemoteException {
		...
		if(!(CS_BUSY || WANTS_TO_ENTER_CS)) {// caseid=0: Receiver is not accessing shared resource and does not want to (send OK to sender)
			caseid = 0;
		}else if(CS_BUSY) {// caseid=1: Receiver already has access to the resource (dont reply but queue the request)
			caseid = 1;
		}else{// caseid=2: Receiver wants to access resource but is yet to - compare own message clock to received message's clock
			caseid = 2;
		}
...
//				int reciver = clock.getClock();
				...
				if(sender < reciver || (sender == reciver && senderid.compareTo(reciverid) < 0)) {
					NodeInterface stub = Util.getProcessStub(procName, port);
					message.setAcknowledged(true);
					stub.onMutexAcknowledgementReceived(message);
